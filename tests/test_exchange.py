...
Every env var this module reads is unset by default, so the first thing each
test does is establish the exact environment it needs via monkeypatch.
The one HTTP server used here binds to 127.0.0.1 — no external network.
"""
...
def test_default_output_dir_prefers_env_over_input_parent(monkeypatch, tmp_path):
    data = tmp_path / "elsewhere" / "in.csv"
    data.parent.mkdir()
    data.write_bytes(CSV_BODY)
    monkeypatch.setenv("MCP_OUTPUT_DIR", str(tmp_path / "shared"))
    assert get_default_output_dir(str(data)) == tmp_path / "shared"
...
def test_default_output_dir_uses_input_parent_when_env_unset(monkeypatch, tmp_path):
    ...
    data.write_bytes(CSV_BODY)
    assert get_default_output_dir(str(data)) == tmp_path
...
def test_public_url_encodes_and_keeps_subdirectories(remote_mode):
    nested = remote_mode / "inbox" / "my report.csv"
    nested.parent.mkdir(parents=True)
    nested.write_bytes(CSV_BODY)
    assert public_url_for(nested) == "https://files.example.test/data/inbox/my%20report.csv"
...
def test_public_url_empty_when_not_configured(monkeypatch, tmp_path):
    ...
    assert public_url_for(tmp_path / "x.html") == ""
...
def test_is_url_only_matches_http_schemes():
    ...
    assert not is_url("workspace:demo/data")
...
def test_resolve_path_rejects_url_when_fetching_disabled(monkeypatch):
    ...
    with pytest.raises(ValueError, match="does not fetch URLs"):
        resolve_path("https://example.test/a.csv")
...
@pytest.mark.parametrize(
    "url",
    [
        "http://127.0.0.1:8820/basic/health",
...
def test_assert_fetchable_allows_private_when_opted_in(monkeypatch):
    monkeypatch.setenv("MCP_FETCH_ALLOW_PRIVATE", "1")
    assert_fetchable("http://127.0.0.1:8820/health")
...
def test_fetch_url_is_cached_within_the_ttl(remote_mode, http_url):
    first = fetch_url(f"{http_url}/data.csv")
    first.write_bytes(b"sentinel")
    assert fetch_url(f"{http_url}/data.csv").read_bytes() == b"sentinel"


def test_fetch_url_refetches_when_cached_file_is_gone(remote_mode, http_url):
    first = fetch_url(f"{http_url}/data.csv")
    first.unlink()
    assert fetch_url(f"{http_url}/data.csv").read_bytes() == CSV_BODY
