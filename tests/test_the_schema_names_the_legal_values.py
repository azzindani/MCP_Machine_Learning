...
for _p in (str(ROOT), str(ROOT / "servers")):
...
def _dispatch_params():
    from servers.ml_advanced import server as adv
    from servers.ml_basic import server as basic
    from servers.ml_medium import server as medium

    for app, label in [(basic.mcp, "ml-basic"), (medium.mcp, "ml-medium"), (adv.mcp, "ml-advanced")]:
...
def test_the_census_finds_them():
    ...
    assert len(found) >= 12, f"only {len(found)} dispatch parameters seen -- did the census break?"
...
def test_the_model_enums_render_from_the_registries():
    """Not a second list: the same objects the trainers validate against."""
    from servers.ml_medium._medium_helpers import ALLOWED_CLASSIFIERS, ALLOWED_REGRESSORS

    found = dict(_dispatch_params())
    assert set(found["ml-basic/train_classifier.model"]["enum"]) == set(ALLOWED_CLASSIFIERS)
    assert set(found["ml-basic/train_regressor.model"]["enum"]) == set(ALLOWED_REGRESSORS)
    both = set(ALLOWED_CLASSIFIERS) | set(ALLOWED_REGRESSORS)
    for key in ("ml-medium/train_with_cv.model", "ml-advanced/tune_hyperparameters.model"):
        assert set(found[key]["enum"]) == both, key


def test_every_task_value_is_one_the_tools_take():
    found = dict(_dispatch_params())
    for key in [k for k in found if k.endswith(".task")]:
        assert set(found[key]["enum"]) == {"classification", "regression"}, key
