...
NAMES = ("basic", "medium", "advanced")
PUBLIC = "https://ml.example.test"
...
out = {}
with TestClient(unified_server.app, base_url=os.environ["ML_PUBLIC_URL"]) as c:
    for name in unified_server._TIERS:
...
def discovery(tmp_path_factory) -> dict:
    ...
    env = {**os.environ, "ML_API_KEY": "test-key-not-a-secret", "ML_PUBLIC_URL": PUBLIC}
    for name in (*NAMES, "domain"):
        env[f"ML_{name.upper().replace('-', '_')}_OAUTH_STATE_DIR"] = str(state / name)
