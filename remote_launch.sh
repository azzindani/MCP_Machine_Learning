...
# process serving all 3 tiers as separate MCP endpoints (/basic/mcp,
# /medium/mcp, /advanced/mcp) — and opens a single Cloudflare Quick Tunnel
# to it. Same idea as azzindani/Folio's launch.sh, and this repo's own
...
cd "$REPO_DIR"
echo "[remote_launch] starting all 3 tiers (one process) on :${PORT}..."
...
if [ -n "$URL" ]; then
  echo ""
  echo "  basic     -> $URL/basic/mcp"
  echo "  medium    -> $URL/medium/mcp"
  echo "  advanced  -> $URL/advanced/mcp"
  echo ""
else
