...
"File not found: ad_data.csv" with the hint "Check file_path is absolute and
the file exists" is the commonest failure a remote caller meets, and the hint
is wrong on both counts: a relative path is read from the data folder, and the
caller cannot look to see what does exist -- it shares no filesystem with this
server. So it guesses again. `Ad_Data.csv` sitting one case-fold away, or
`churn_model.pkl` for a caller who remembered `churn_model.joblib`, is exactly
the answer it needed and the only thing that could not see it was the server.

Every tool that takes a dataset or a model reports a missing one, each with its
own copy of that hint, so this is a choke point on the response rather than one
edit per tool: any failure whose error names a missing file or model gains
`did_you_mean` and a hint built from it.
...
from shared.file_utils import PathOutsideRootError, confine, data_root, paths_confined

_MISSING = re.compile(r"^(?:[A-Za-z]+ )?(?:file|model) not found: (?P<name>.+)$", re.IGNORECASE)
...
def _score(wanted: str, candidate: str) -> float:
    ...
    if w == c:
        return 3.0  # the same name in other letters: Ad_Data.csv for ad_data.csv
...
def _suggesting(fn: Any, name: str) -> Any:
    ...
    def suggesting(*a: Any, **kw: Any) -> Any:
        # A refused path is an answer, not a crash. Eight resolvers (clustering
