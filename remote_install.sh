#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# MCP_Machine_Learning — remote bootstrap (Google Colab / any fresh Linux VM,
# no Docker needed). Installs uv, clones/updates this repo, syncs Python
# deps for all 3 tiers (basic/medium/advanced).
#
# Companion to remote_launch.sh. Same idea as azzindani/Folio's install.sh,
# adapted for a uv/Python project instead of npm.
#
# Usage:
#   REPO_DIR=/content/MCP_Machine_Learning ./remote_install.sh
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail

REPO_DIR="${REPO_DIR:-/content/MCP_Machine_Learning}"
REPO_URL="${REPO_URL:-https://github.com/azzindani/MCP_Machine_Learning.git}"
...
cd "$REPO_DIR"
uv sync 2>&1 | tail -5
echo "✓ MCP_Machine_Learning installed (basic/medium/advanced)"
