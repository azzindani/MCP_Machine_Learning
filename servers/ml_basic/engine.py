...
def predict_single(model_path: str, input_data: str) -> dict:
    ...
    if not feature_columns:
        return _error("Model has no feature_columns in metadata.", "Retrain the model to regenerate metadata.")
    ...
    try:
        ...
        return _error(f"Prediction failed: {exc}", "Check input values match training data types.")
...
def split_dataset(
    ...
    if test_size <= 0 or test_size >= 1:
        return _error(f"test_size={test_size} must be in (0, 1).", "Use e.g. test_size=0.2 for 20% test.")
    ...
    if stratify_column:
        if stratify_column not in df.columns:
            return _error(f"Column '{stratify_column}' not found.", "Use inspect_dataset() to see column names.")
